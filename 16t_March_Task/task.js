 ...
 */
class Vehicle {
    constructor(brand, model, engineType, vin) {
        this.brand = brand;
        this.model = model;
...
class Car extends Vehicle {
    constructor(brand, model, engineType, vin, numberOfDoors) {
    ...
    drive(distance) {
        ...
        console.log(`Driving ${distance} miles. Total mileage: ${this.mileage} miles`);
    }
    // METHOD that accesses PROTECTED property directly
    getMaintenanceStatus() {
        // ✅ Can directly access protected property
        const dayssinceService = Math.floor((new Date().getTime() - this.lastServiceDate.getTime()) /
            (1000 * 3600 * 24));
...
class Motorcycle extends Vehicle {
    constructor(brand, model, engineType, vin, engineSize) {
        super(brand, model, engineType, vin);
...
const myMotorcycle = new Motorcycle("Harley-Davidson", "Street 750", "V2", "HD123", 750);
...
console.log("   ❌ Cannot access protected properties directly from outside the class hierarchy");
// ============================================================================
...
class BankAccount {
    constructor(accountNumber, accountHolder, initialBalance, pin) {
        this.transactionHistory = []; // Private - internal tracking
        ...
        this.transactionHistory.push(`Account created with balance: $${initialBalance}`);
    }
    ...
    getRecentTransactions(enteredPin, count = 5) {
        if (!this.verifyPin(enteredPin)) {
            return "❌ Incorrect PIN!";
        }
...
class SavingsAccount extends BankAccount {
    ...
    applyMonthlyInterest() {
        ...
        console.log(`Interest applied at rate: ${(this.interestRate * 100).toFixed(1)}% (Account created: ${this.accountCreatedDate.toLocaleDateString()})`);
    }
    ...
    tryAccessPrivate() {
        ...
        console.log("Cannot access PRIVATE properties from parent class in subclass!");
    }
}
...
const savingsAccount = new SavingsAccount("SAV789012", "Jane Doe", 5000, "5678");
